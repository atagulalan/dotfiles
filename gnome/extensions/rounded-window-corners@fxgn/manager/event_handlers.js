...
import { computeBounds, computeShadowActorOffset, computeWindowContentsOffset, getRoundedCornersCfg, getRoundedCornersEffect, shouldEnableEffect, updateShadowActorStyle, windowScaleFactor, } from './utils.js';
export function onAddEffect(actor) {
    ...
    actor.add_effect_with_name(ROUNDED_CORNERS_EFFECT, new RoundedCornersEffect());
 ...
 */
function getBorderColor(win, config) {
...
function refreshRoundedCorners(actor) {
    ...
    effect.updateUniforms(windowScaleFactor(win), cfg, computeBounds(actor, windowContentOffset), getBorderColor(win, cfg));
